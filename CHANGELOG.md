...
- Runtime hardening with no-optional-lock Git inspection, global MCP tool-response budgeting, Composer metadata-driven lazy service invalidation, `--no-git` propagation, complete read-only doctor checks, and regression tests for no host execution, arbitrary shell, mutation, or network primitives.
- Release-candidate validation with official-SDK STDIO protocol coverage for handshake/modern eras, integration/ecosystem/security suites, representative PHPBench workloads, Linux/Windows PHP 8.4/8.5 CI, PHPForge release gates, Infbyte `require-dev`/zero-footprint checks, and complete README/security guidance.
