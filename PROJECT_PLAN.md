...
- `e983e38e` — static Foundation/Webrick route inspection over authoritative project route files, installed Foundation OAuth registration and optional Webrick attributes; installed route-file/verb/resource semantics are parsed from the installed packages instead of copied registries; nested/preset groups, resource expansion, callable handlers, middleware/options, conditional/dynamic registrations and parse diagnostics are bounded and non-bootstrapping. Focused route fixtures are present; execution is deferred to the later dedicated verification pass.
