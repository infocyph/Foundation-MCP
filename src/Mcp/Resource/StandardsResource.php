...
final readonly class StandardsResource
{
    ...
    public function execute(): string
    {
        ...
        foreach ([...self::DOCUMENTS, ...self::CONFIGS] as $path) {
            ...
            if (count($sources) >= self::MAX_SOURCES) break;
        }
        foreach (['infocyph/foundation', 'infocyph/phpforge'] as $package) {
            if (count($sources) >= self::MAX_SOURCES || $this->services->composer()->package($package)?->installPath === null) continue;
            foreach ([...self::DOCUMENTS, ...self::CONFIGS] as $path) {
    ...
    {
        try { $sources[] = $this->source('project', $path, $this->services->reader()->project($path, 1, 120)); } catch (Throwable) {}
    }

    /** @param list<array<string,mixed>> $sources */
    private function package(string $package, string $path, array &$sources): void
    {
        try { $sources[] = $this->source($package, $path, $this->services->reader()->package($package, $path, 1, 120)); } catch (Throwable) {}
    }
