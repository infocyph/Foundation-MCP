...
final class ImpactAnalyzer
{
    ...
    private function file(string $path): void
    {
        ...
        $this->add('file', $relative, 'exact', 'target_file', $relative, null);
        $found = false;
        ...
        try {
            foreach ($this->tests->forFile($relative, 50) as $test) {
                ...
                $this->add('test', $test['path'], $test['confidence'], 'related_test:'.implode(',', $test['reasons']), $test['path'], null);
            }
    ...
    private function module(string $module): void
    {
        ...
        foreach ($resolved['config'] as $config) {
            ...
            $this->add('config', $config, is_file($this->project->root.DIRECTORY_SEPARATOR.'config'.DIRECTORY_SEPARATOR.$config) ? 'exact' : 'lexical', 'module_config', $path, null);
        }
    }
    ...
    private function changes(): void
    {
        ...
        foreach ($workspace['files'] as $file) {
            ...
            $this->add('file', $file['path'], 'exact', 'workspace_'.$file['change'], $file['path'], null);
        }
        ...
        foreach ($workspace['affected_tests'] as $test) {
            ...
            $this->add('test', $test, 'resolved', 'workspace_related_test', $test, null);
        }
